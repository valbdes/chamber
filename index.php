...
while ($i < $hue_arr_length) {
    echo ('<p>' . 'Это цвет' . ' ' . $hue_array[$i]['color'] . ' ' . 'он из категории' . ' ' . $hue_array[$i]['category'] . '.' . 'Тип цвета' . ' ' . $hue_array[$i]['type'] . '.' . 'В данном цвете следующии кода' . ' ' .  '[{$code_separ}]' . ' ' . 'и такой hex код -' . $hue_array[$i]['code']['hex'] . '</p>');
    foreach ($hue_array[$i]['code']['rgba'] as $codergba) {
        echo "[{$codergba}]";
    }$i++;
};
